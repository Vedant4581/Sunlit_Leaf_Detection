...
class ResNetV2(nn.Module):
    ...
    def forward(self, x):
        ...
        # Stem: 512?256
        x = self.root(x)                # 256×256, C=width
        features.append(x)              # keep 256

        # Pool to ~128 (use padding=1 to land exactly on /4)
        x = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)(x)

        # block1: ? 128×128, C=width*4
        x = self.body[0](x)
        features.append(x)

        # block2: ? 64×64, C=width*8
        x = self.body[1](x)
        features.append(x)

        # block3: ? 32×32, C=width*16
        x = self.body[2](x)
        features.append(x)

        # Return skips ordered from low?high resolution: [32, 64, 128, 256]
